...
object Flights {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val airports = flightsRDD.map(flight => (flight.originId.toLong, flight.originCode)).distinct
    val airport = airports.take(1)
    println(airport.apply(0)._1 + "  " + airport.apply(0)._2)

    val routes = flightsRDD.map(flight => ((flight.originId, flight.destId), flight.dist, flight.arrivalDelay)).distinct

    routes.cache
    val edges = routes.map { case ((originId, destId), distance, arrivalDelay) => Edge(originId.toLong, destId.toLong, (distance, arrivalDelay)) }
    ...
    val airportMap = airports.map { case ((originId), originCode) => (originId -> originCode) }.collect.toList.toMap
